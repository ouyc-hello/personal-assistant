...
class MemoryCandidate(Base):
    ...
    id: Mapped[str] = mapped_column(GUID(), primary_key=True, default=new_id)
    # These nullable/legacy-compatible fields keep ORM writes working against
    # installations created by the earlier memory SDK schema.
    thread_id: Mapped[str | None] = mapped_column(GUID(), nullable=True)
    source_message_id: Mapped[str | None] = mapped_column(GUID(), nullable=True)
    title: Mapped[str] = mapped_column(Text, nullable=False, default="")
    candidate_type: Mapped[str] = mapped_column(String(50), nullable=False, default="context")
    sdk_memory_id: Mapped[str | None] = mapped_column(String(200), nullable=True)
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
...
class AuditEvent(Base):
    ...
    entity_type: Mapped[str] = mapped_column(String(200), nullable=True)
    entity_id: Mapped[str] = mapped_column(String(200), nullable=True)
    # Legacy audit schema used aggregate_* and actor_type. They are nullable in
    # the formal schema, but populated by _audit for old PostgreSQL databases.
    aggregate_type: Mapped[str | None] = mapped_column(String(200), nullable=True)
